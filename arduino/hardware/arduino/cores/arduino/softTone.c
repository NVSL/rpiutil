...
static pthread_t noToneThreads [MAX_PINS] ;
static unsigned long durations[MAX_PINS];
...
/*
 * noToneThread
 *	Thread to stop the tone when time out.
 *********************************************************************************
 */
static PI_THREAD (noToneThread)
{
  int pin;
...
void softToneStop (int pin)
{
  if (threads [pin] != 0)
  {
    ...
    if(noToneThreads[pin] != 0){
        pthread_cancel (noToneThreads [pin]) ;
        pthread_join   (noToneThreads [pin], NULL) ;
        noToneThreads[pin] = 0;
    }
    threads [pin] = 0 ;
    digitalWrite (pin, LOW) ;
...
{
    if(softToneCreate(pin) == -1)   return;
    softToneWrite(pin, frequency);
    if(duration){
        newNoTonePin = pin;
        durations[pin] = duration;
        pthread_t myThread ;
        pthread_create (&myThread, NULL, noToneThread, NULL) ;
        noToneThreads [pin] = myThread ;
    }
}
